...
- Refactored the current `auto_ur` package into a structured action library
  instead of adding a separate first-draft package.
- Added structured result and failure contracts in `auto_ur/core/`:
  `PrimitiveResult`, `SkillResult`, `RecoveryResult`, `Failure`, and
  `FailureType`.
- Added `auto_ur/world_model.py` for symbolic object, location, and robot
  state used by skills.
- Preserved the existing MoveItPy plan-only motion backend in
  `auto_ur/primitives/arm_motion.py`, while updating motion primitives to
  return `PrimitiveResult`.
- Added deterministic primitive stubs for gripper control, perception, and
  reachability checks under `auto_ur/primitives/`.
- Added class-based skill infrastructure in `auto_ur/skills/`, including
  `Skill`, `PickObject`, and `PlaceObject`.
- Updated `auto_ur/skills/pick_place_demo.py` so the existing demo wrapper runs
  through the structured `PickObject` and `PlaceObject` flow while preserving
  the six-segment trajectory summary expected by the demo node.
- Updated `auto_ur/registry/` so the default registry still exposes existing
  primitive/demo handlers and can instantiate skill classes from action
  dictionaries.
- Added `test/test_action_library_contracts.py` and updated
  `test/test_moveit_first_demo.py` for structured result, world-model,
  registry, skill, and plan-only safety coverage.
- Updated `docs/architecture.md` to describe the current structured
  action-library layers, primitive/skill boundaries, local recovery scope,
  registry behavior, and RViz node flow.
- Verification on the Windows checkout:
  - `pytest` and `python -m pytest` could not run because `pytest` is not
    installed in the active Python environment.
  - Import/smoke checks passed for registry construction, action-dictionary
    skill instantiation, and `PickObject` followed by `PlaceObject`.
  - A fake-arm smoke check passed for `pick_and_place_demo`, confirming the
    wrapper still reports the sequence `pre_pick`, `pick`, `lift`,
    `pre_place`, `place`, and `retreat`.
  - A safety grep found no hardware trajectory execution calls in active
    package code.

### 06/11 15:49

- Updated `README.md` from a placeholder into user-facing documentation for the
  current structured action-library package.
- Documented the current demos, package layers, build/run commands, structured
  registry usage, focused tests, full `colcon test`, and the plan-only safety
  boundary.
- Fixed style issues found by WSL package testing:
  - import ordering in `auto_ur/primitives/__init__.py`,
    `auto_ur/registry/default_actions.py`, and
    `launch/demo_plan_only.launch.py`
  - quote style in `auto_ur/nodes/gripper_object_demo.py`
  - D213 module docstring shape in `auto_ur/primitives/gripper.py`,
    `auto_ur/primitives/perception.py`, and `auto_ur/skills/base.py`
- Built and tested from a temporary WSL colcon workspace at `/tmp/auto_ur_ws`
  with the package symlinked to the Windows checkout.
- Ran WSL focused tests:
  `python3 -m pytest -q test/test_action_library_contracts.py test/test_moveit_first_demo.py`.
  Result: 15 passed, 1 skipped.
- Ran WSL build:
  `colcon build --packages-select auto_ur --symlink-install --allow-overriding auto_ur`.
  Result: passed.
- Ran WSL package tests:
  `colcon test --packages-select auto_ur --event-handlers console_direct+`
  and `colcon test-result --verbose`.
  Result: 19 tests, 0 errors, 0 failures, 2 skipped.
- Ran WSL non-RViz demo smoke:
  `timeout 50s ros2 launch auto_ur demo_plan_only.launch.py rviz:=false`.
  Result: launch exited with code 0; registered actions included
  `PickObject` and `PlaceObject`; `move_to_named_pose`, `move_to_joint_state`,
  `move_to_pose`, and `pick_and_place_demo` all succeeded; all six pick/place
  trajectories were published for playback.
...
## Superseded Stage 2 Reminder

The older Stage 2 note said the package should introduce abstract interfaces
and avoid MoveIt. That direction has been superseded by the MoveIt-first
plan-only implementation and the later structured action-library refactor.

The current boundary is:

- keep MoveItPy planning inside primitives
- keep skills independent from direct ROS 2 and MoveIt calls
- keep hardware execution, global failure routing, LLM planning, TAMP, and
  behavior-tree execution outside this package for now
